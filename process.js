...
const mapRelations = (entities, relations, files) => {
  ...
  relations.forEach(relation => {
    if (relation.junction_field === null) {
      ...
      if (!co || !cm) return;
      info(`Found One-To-Many relation: ${co} -> ${cm}`); // If the relation hasn't been defined in both collections, fall back
...
const mapFilesToNodes = (files, collections, entities) => {
